const fs = require('fs');
const fse = require('fs-extra');

const makeDir = function myFunction(directory,username){

  //If user _data is not created already
  if(!fs.existsSync("user_data"))
  {
      //create folder 'user_data'
    fs.mkdirSync("user_data");
  }
  if(!fs.existsSync('./user_data/' + username))
  {
    fs.mkdirSync('./user_data/' + username);
  }
  try {
    fs.mkdirSync('./user_data/' + username + "/" +  directory);
    return true;
  } 
  catch (e) {
    return (e);
  }
  
}



const touchfile = function myFunction(directory,file,username){

  //If user _data is not created already
  if(!fs.existsSync("user_data/directory"))
  {
      //create folder 'user_data'
    fs.mkdirSync("user_data/directory/");
  }
  if(!fs.existsSync('./user_data/directory/' + username))
  {
    fs.touchSync('./user_data/directory/' + username);
  }
  try {
    fs.touchSync('./user_data/directory/' + username + "/" +  file);
    return true;
  } 
  catch (e) {
    return (e);
  }
  
}

const deleteDir = function myFunction(directory,username){
  
  try{
      if(!fse.existsSync('./user_data/' + username + "/" +  directory))
      {
        return false;
      }
     fse.removeSync('./user_data/' + username + "/" +  directory);
     return true;
  }
  catch (e) {
    return (e);
  }
  
}

module.exports = {
  makeDir,
  deleteDir,
  touchfile
}
